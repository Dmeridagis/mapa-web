		...
		<div class="row">
			<div class="col-md-3" style="padding-left: 30px;">
			 <form method="post" id="insert_form_sitio">
						...
						<br />
						<label> Tipos</label>
						<input type="text" name="txttipositio" id="txttipositio" class="form-control"  />
						<br />
						<label> Sitio de interes</label>
						<input type="text" name="txtnombresitio" id="txtnombresitio" class="form-control"  placeholder="001"/>
						<br />
						<label> Coordenadas</label>
						<textarea name="txtgeositio" id="txtgeositio" class="form-control" rows="10" readonly ></textarea>
						<br />
						<input type="submit" name="insertsitio" id="insertsitio" value="Actualizar" class="btn btn-success" />
					</form>
			</div>
			<div class="col-md-9">
...
<script>
  ...
  var osm = L.tileLayer('http://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', {
    ...
    map.addControl(osmGeocoder);
		...
		var jsonPHP=JSON.parse('<?php echo $si;?>');
...
map.on('draw:deleted', function () {
 ...
 if (jsonPHP){
	 ...
	 $('#txttipositio').val(jsonPHP['features'][0]['properties']['tipo']);
	 $('#txtnombresitio').val(jsonPHP['features'][0]['properties']['nombre']);
	 $('#txtgeositio').val(JSON.stringify(jsonPHP['features'][0]['geometry']));
 }
