...
var reclamosLayer = L.geoJson(null);

// Definir la capa de reclamos
var reclamos = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    // Crear un marcador personalizado para cada reclamo con un ícono específico
    return L.marker(latlng, {
      icon: L.icon({
        iconUrl: "assets/img/ReclamoS.png",  // Ícono de reclamo (cambiar según diseño)
        iconSize: [24, 28],  // Tamaño del ícono
        iconAnchor: [12, 28],  // Punto de anclaje del ícono
        popupAnchor: [0, -25]  // Anclaje del popup
      }),
      title: feature.properties.descripcion,  // Descripción del reclamo
      riseOnHover: true  // Hace que el marcador se eleve al pasar el cursor
    });
  },
  
  onEachFeature: function (feature, layer) {
    // Agregar reclamo a la lista de búsqueda
    buscarSitio.push({
    ...
    // Vincular un popup con opciones para agregar, editar y eliminar reclamos
    layer.bindPopup("<div style=text-align:center><h3>"+
      '<button type="button" onclick="document.getElementById('+"'nuevoReclamoModal'"+').style.display='+"'block'"+'; setTimeout(mapReclamo.invalidateSize(), 1000);" class="btn btn-success" data-toggle="modal" data-target="#nuevoReclamoModal">Estado</button>&ensp;'+
      '<a href="report/editaReclamo.php?codigo='+ feature.properties.id +'" class="btn btn-default" >Responder</a>&ensp;'+
      '<a href="report/eliminarreclamo.php?codigo='+ feature.properties.id +'" class="btn btn-danger" onclick="return confirm('+"'seguro?'"+')">Eliminar</a>'+
      "</h3><hr style='height:2px;border-width:0;color:gray;background-color:gray'></div><table><tr><td>Mensaje: "+feature.properties.mensaje+
    "</td></tr><tr><td>Área:"+feature.properties.tipo+
    "</td></tr><tr><td>imagen:"+feature.properties.imagen+
      "</td></tr><tr><td>Fecha: "+feature.properties.fecha+  // Mostrar más detalles del reclamo
      "</td></tr></table>",
      { minWidth: 250, maxWidth: 300 }  // Establecer el tamaño del popup
    );
  }
...
// Cargar datos de los reclamos desde un archivo JSON y agregarlos a la capa 'reclamos'
$.getJSON("report/reclamos.php", function (data) {
  reclamos.addData(data);  // Añadir los datos de reclamos a la capa
  map.addLayer(reclamosLayer);  // Añadir la capa de reclamos al mapa
});
...
var groupedOverlays = {
  "Activar Reclamos": {
    "<img src='assets/img/Reclamos.png' width='24' height='28'>&nbsp;Reclamos": reclamos,  // Reclamos
  },
  "Capas": {
